...
if (Meteor.isClient) {
  ...
  Template.multiLine.onRendered(function () {
    ...
    // Test_DATA.find({}, {sort: {_id: 1}}).map(function (doc, index, cursor) {
    Tags_db.find().map(function (doc, index, cursor) {
      color.domain(
        d3.keys(doc).filter(function(key) { return key !== "_id"; })
      );
      ...
      var cities = color.domain().map(function(name) {
        return {
          ...
          values: cursor.fetch().map(function(d) {
            return {date: parseDate(String(d._id)), temperature: +d[name]};
          })
        };
      });
